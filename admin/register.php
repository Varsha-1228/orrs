...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($firstname_err) && empty($lastname_err) && empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO users (firstname, middlename, lastname, username, password, type, status, date_added) VALUES (?, ?, ?, ?, ?, 2, 1, NOW())";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_firstname, $param_middlename, $param_lastname, $param_username, $param_password);
